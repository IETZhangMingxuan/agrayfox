...
$(function () {
            // 0.进入网页后等待1秒钟机器人自动回复
            setTimeout(function () {
                $(`<p class="left">你好呀,有什么不懂的问题可以来问我</p>`)
                    .appendTo('.talkList');
            }, 1000);
            // 1.清除聊天
            ...
            $('input').on('keyup', function (e) {
                if (e.keyCode == 13) {
                    if ($('input').val() !== '') {
                        ...
                        $('input').val('');
                        /* 重点:创建一个虚拟元素占位 */
                        $('<p class="deleteP right" style="visibility:hidden"></p>').appendTo(
                            '.talkList');
                        ...
                        try {
                            axios({
                                ...
                                $('.deleteP').remove();
                                $(`<p class="left">${response.data.text}</p>`).appendTo(
                                    '.talkList');
                                $('<p class="deleteP right" style="visibility:hidden"></p>')
                                    .appendTo('.talkList');
                                // 重点:设置滚动条每次都在最新消息的底部
            ...
            function sendMessage() {
                if ($('input').val() !== '') {
                    ...
                    axios({
                        ...
                        $(`<p class="left">${result}</p>`).appendTo(
                            '.talkList')
                    })
                }
            }
