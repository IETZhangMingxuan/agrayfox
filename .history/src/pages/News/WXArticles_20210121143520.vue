<template>
  <div id="hotweibo" class="animate__animated animate__slideInLeft">
    ...
    <div class="main">
      <h3 v-show="!this.resultList.length" class="loading">努力加载中...</h3>
      <ul class="hotWeiboList">
        <li
          v-for="(item, index) in this.resultList.slice(currentIndex, currentIndex + pageSize)"
          :key="index"
        >
    ...
    searchVXArticles(wxsearch,resultList) {
      axios
        ...
        .then(response => {
          ...
          console.log("我已成功取得微信文章数据为:" + result);
