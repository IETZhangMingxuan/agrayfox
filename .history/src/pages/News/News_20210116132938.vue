  ...
  <div id="news">
    ...
    <div class="recommend">
      ...
      <div class="virusReport animate__animated animate__lightSpeedInRight">
        ...
        <div class="virusNews" v-for="(p,index) in VirusNumList[0].news" :key="index">
          <a :href="p.sourceUrl" target="_blank" class="title">{{p.title}}</a>
          <p class="author">{{p.infoSource}}</p>
          <p class="time">{{p.pubDateStr}}</p>
          <p class="main">{{p.summary}}</p>
        </div>
      </div>
      <!-- echarts地图模块 -->
      <div></div>
