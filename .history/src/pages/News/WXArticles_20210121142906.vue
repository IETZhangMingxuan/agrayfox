...
export default {
  ...
  methods: {
    ...
    searchVXArticles(wxsearch) {
      console.log(this.wxsearch);
      this.getWXArticlesList();
      axios({
        methods: "GET",
        url: `http://api.tianapi.com/txapi/wxsearch/index?key=4428d8e64916b6207baf857dac31f424&word=${this.wxsearch}`
      });
      axios
        .get(url)
        .then(response => {
          const result = response.newslist;
