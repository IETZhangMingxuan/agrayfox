  ...
  <div id="news">
    <div class="navbar">
      <ul class="newsList animate__animated animate__slideInDown" @click="changeVisible">
        <li v-for="(item,index) in newsNavbarList" :key="index" :data-router-name="item.routerName">
          <div class="swiperBlock" :v-show="isVisible" :data-router-name="item.routerName"></div>
          <router-link :to="`/news/${item.routerName}`" class="singleLink" :data-router-name="item.routerName">{{item.name}}</router-link>
        </li>
        <!-- <li
...
export default {
      ...
      newsNavbarList: [
        {
          id: 1,
          name: "微信热文",
          routerName:"/news/WXNews"
        },
        {
          id: 2,
          name: "网易新闻",
          routerName:"/news/WYNews"
        },
        {
          id: 3,
          name: "游戏新闻"
  ...
  methods: {
    ...
    changeVisible(e){
      // this.isVisible = true;
      const {routerName} = e.target.dataset;
      console.log(e.target.dataset.routerName);
