    ...
    <div class="bloglist-body">
      <div class="bloglist-left">
        <div class="bloglist-main">
          <ul>
            <li v-for="news in filterList" :key="news.id">
          ...
          <button
            class="getmore"
            v-show="this.filterList[0] !== undefined && this.newsList.length >= 3"
          >
            加载更多
          </button>
          <div class="loading" v-show="this.newsList.length === 0">正在加载中...</div>
        </div>
      </div>
  ...
  methods: {
    ...
    deleteBlog(index) {
      if (this.filterList.length > 1) {
        this.filterList = this.newsList.filter(item => {
          return item.id !== index;
        });
      } else {
        alert("已经不可以再删除了,你至少看一条吧!");
      }
    }
  },
  async mounted() {
