      ...
      <ul class="videoList animate__animated animate__flipInY">
        ...
        >
          <div class="router_link" @click="playNow(p, playUrl, playImg, videoTitle)">
            <img
              :src="p.data.cover.detail"
              alt=""
              class="posterImg"
              v-if="p.data.cover !== undefined"
            />
          </div>
          <div class="title">
            <img :src="p.data.tags[0].bgPicture" alt="" class="authorImg" v-if="p.data" />
            <p class="videoTitle" v-if="p.data.author !== undefined">{{ p.data.author.name }}</p>
          </div>
          <p class="videoDesc">{{ p.data.author.description !== undefined }}</p>
        </li>
      </ul>
...
export default {
  ...
  methods: {
    ...
    async getKYVideoList(sendAxios) {
      var ajaxRandomNum = parseInt(Math.random() * (130000 - 120000) + 120000);
      console.log(ajaxRandomNum, this.sendAxios);
      this.sendAxios = "加载中";
      await axios
      ...
      if (this.KYVideoList[0].data == undefined || this.KYVideoList[0].data.tags[0] == undefined || his.KYVideoList[0].data.name == undefined) {
