      ...
      <ul class="list" v-infinite-scroll="load" infinite-scroll-disabled="disabled">
        <li v-for="(i, index) in DouyinMusicList" class="list-item" :key="index" @click="openTips">
          ...
          <div class="musicAuthor">
            <span><a :href="`https://y.qq.com/portal/search.html#page=1&searchid=1&remoteplace=txt.yqq.top&t=song&w=${i.author}`" target="_blank">{{ i.author }}</a></span>
          </div>
          <div class="musicHref">
            <a :href="`https://y.qq.com/portal/search.html#page=1&searchid=1&remoteplace=txt.yqq.top&t=song&w=${i.title}`" target="_blank">{{ i.title }}</a>
          </div>
  ...
  methods: {
    ...
    openTips(){
      this.$notify({
          ...
          message: '<span style="color:#333;font-size:16px;font-family:"consolas";line-height:15px">请点击歌曲名听歌曲<br/><div style="margin-top:8px;margin-bottom:0px">或点击歌手名搜索歌手相关信息</div></span>',
          title:"提示：",
          offset: 250,
          showClose: false,
          type:"success"
