      ...
      <ul class="videoList animate__animated animate__flipInY">
        ...
        >
          <div class="router_link" @click="playNow(p, playUrl, playImg, videoTitle)">
            <img :src="p.data.cover.detail" alt="" class="posterImg" />
          </div>
          <div class="title">
...
export default {
  ...
  methods: {
    ...
    async getKYVideoList() {
      ...
      await this.playImg = this.KYVideoList[0].data.cover.detail;
      await this.videoTitle = this.KYVideoList[0].data.title;
      await this.playUrl = this.KYVideoList[0].data.playUrl;
    },
    async mounted() {
      this.getKYVideoList();
    }
