<template>
  <div id="kaiyan">
      ...
      <ul class="videoList animate__animated animate__flipInY">
        ...
        >
          <div class="router_link" @click="playNow(p, playUrl, playImg, videoTitle)">
            <img :src="p.data.cover.detail" alt="" class="posterImg" v-if="p.data.cover.detail"/>
          </div>
          <div class="title">
            <img :src="p.data.tags[0].bgPicture" alt="" class="authorImg" v-if="p.data.tags[0].bgPicture"/>
            <p class="videoTitle" v-if="p.data.author.name">{{ p.data.author.name }}</p>
          </div>
    ...
    <div class="videoMain">
      ...
      ></video>
      <p class="kaiyan">来自开眼视频————最有深度的短视频</p>
    </div>
  </div>
</template>
<script>
