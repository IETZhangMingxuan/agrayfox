<template>
  <div id="hot" class="animate__animated animate__slideInLeft">
    <div class="header">
      <div class="titleImg">
        <img :src="WeiboNewsList.site.attrs.logo" alt="" style="width:25px;height:25px" />
      </div>
      <span class="title">{{ titleName }}</span>
      <div @click="changeIndex(currentIndex, totalSize, pageSize)">
        <p class="change"><i class="iconfont icon-exchange"></i>换一批</p>
      </div>
    </div>

    <div class="main">
      <h3 v-show="!WeiboNewsList.site.subs[0].items.length" class="loading">努力加载中...</h3>
      <ul class="hotWeiboList">
        <li
          v-for="(item, index) in WeiboNewsList.site.subs[0].items.slice(
            currentIndex,
        ...
        >
          ...
          <a :href="`https://s.weibo.com/weibo?q=${item.title}`" target="_blank">{{ item.title }}</a>
...
export default {
  ...
  computed: {
    ...mapState({
      WeiboNewsList: state => state.anyknow.WeiboNewsList
    })
  },
  methods: {
