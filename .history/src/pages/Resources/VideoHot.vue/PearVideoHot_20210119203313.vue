<template>
  <div id="hot" class="animate__animated animate__bounceIn">
    <div class="header">
      <div class="titleImg">
        <img :src="PearVideoList.site.attrs.logo" alt="" style="width:50px;height:25px" />
      </div>
    ...
    <div class="main">
      <h3 v-show="!PearVideoList.site.subs[0].items.length" class="loading">努力加载中...</h3>
      <ul class="hotList">
        <li
          v-for="(item, index) in PearVideoList.site.subs[0].items.slice(
            currentIndex,
        ...
        >
          ...
          <a
            :href="`https://www.pearvideo.com/search.jsp?start=0&k=${item.title}`"
            target="_blank"
            class="mainText"
            >{{ item.title }}</a
          >
...
export default {
  data() {
    return {
      ...
      pageSize: 7,
      /* 标题 */
      titleName: "梨视频热点(点进之后要按回车键)"
    };
  },
  computed: {
    ...mapState({
      PearVideoList: state => state.anyknow.PearVideoList
    })
  },
  methods: {
