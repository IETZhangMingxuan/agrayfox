<template>
  <div id="hot" class="animate__animated animate__slideInLeft">
    <div class="header">
      <div class="titleImg">
        <img :src="QdailyNewsList.site.attrs.logo" alt="" style="width:50px;height:25px" />
      </div>
    ...
    <div class="main">
      <h3 v-show="!QdailyNewsList.site.subs[0].items.length" class="loading">努力加载中...</h3>
      <ul class="hotWeiboList">
        <li
          v-for="(item, index) in QdailyNewsList.site.subs[0].items.slice(
            currentIndex,
        ...
        >
          ...
          <a :href="`https://www.zhihu.com/search?type=content&q=${item.title}?`" target="_blank" class="mainText">{{
            item.title
          }}</a>
...
export default {
  ...
  computed: {
    ...mapState({
      QdailyNewsList: state => state.anyknow.QdailyNewsList
    })
  },
  methods: {
