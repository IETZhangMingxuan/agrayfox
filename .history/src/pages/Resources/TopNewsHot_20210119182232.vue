<template>
  <div id="hot" class="animate__animated animate__bounceIn animate__bounceOut">
    <div class="header">
      <div class="titleImg">
        <img :src="TopNewsList.site.attrs.logo" alt="" style="width:25px;height:25px" />
      </div>
    ...
    <div class="main">
      <h3 v-show="!TopNewsList.site.subs[0].items.length" class="loading">努力加载中...</h3>
      <ul class="hotWeiboList">
        <li
          v-for="(item, index) in TopNewsList.site.subs[0].items.slice(
            currentIndex,
        ...
        >
          ...
          <a :href="`https://www.toutiao.com/search/?keyword=${item.title}`" target="_blank">{{ item.title }}</a>
        </li>
      </ul>
    </div>
    <div class="tag"></div>
...
export default {
  ...
  computed: {
    ...mapState({
      TopNewsList: state => state.anyknow.TopNewsList
    })
  },
  methods: {
