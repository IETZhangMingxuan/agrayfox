<template>
  <div id="hot" class="animate__animated animate__bounceIn">
    <div class="header">
      <div class="titleImg">
        <img :src="CnBetaNewsList.site.attrs.logo" alt="" style="width:50px;height:25px" />
      </div>
    ...
    <div class="main">
      <h3 v-show="!CnBetaNewsList.site.subs[0].items.length" class="loading">努力加载中...</h3>
      <ul class="hotList">
        <li
          v-for="(item, index) in CnBetaNewsList.site.subs[0].items.slice(
            currentIndex,
        ...
        >
          ...
          <a :href="`https://cn.bing.com/search?&q=site:cnbeta.com%20${item.title}?`" target="_blank" class="mainText">{{
            item.title
          }}</a>
...
export default {
  ...
  computed: {
    ...mapState({
      CnBetaNewsList: state => state.anyknow.CnBetaNewsList
    })
  },
  methods: {
