<template>
  <div id="hot" class="animate__animated animate__bounceIn">
    <div class="header">
      <div class="titleImg">
        <img :src="TSKeNewsList.site.attrs.logo" alt="" style="width:50px;height:25px" />
      </div>
    ...
    <div class="main">
      <h3 v-show="!TSKeNewsList.site.subs[0].items.length" class="loading">努力加载中...</h3>
      <ul class="hotWeiboList">
        <li
          v-for="(item, index) in TSKeNewsList.site.subs[0].items.slice(
            currentIndex,
        ...
        >
          ...
          <a :href="`https://36kr.com/search/articles/${item.title}?`" target="_blank" class="mainText">{{
            item.title
          }}</a>
...
export default {
  ...
  computed: {
    ...mapState({
      TSKeNewsList: state => state.anyknow.TSKeNewsList
    })
  },
  methods: {
