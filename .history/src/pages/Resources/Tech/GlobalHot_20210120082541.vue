<template>
  <div id="hot" class="animate__animated animate__bounceIn">
    <div class="header">
      <div class="titleImg">
        <img src="https://ss1.bdstatic.com/70cFvXSh_Q1YnxGkpoWK1HF6hhy/it/u=3761769582,1449663731&fm=26&gp=0.jpg" alt="" style="width:25px;height:25px" />
      </div>
    ...
    <div class="main">
      <h3 v-show="!GlobalNewsList.site.subs[0].items.length" class="loading">努力加载中...</h3>
      <ul class="hotWeiboList">
        <li
          v-for="(item, index) in GlobalNewsList.site.subs[0].items.slice(
            currentIndex,
        ...
        >
          ...
          <a :href="`https://huanqiukexue.com/plus/search.php?kwtype=0&q=${item.title}`" target="_blank">{{ item.title }}</a>
...
export default {
  ...
  computed: {
    ...mapState({
      GlobalNewsList: state => state.anyknow.GlobalNewsList
    })
  },
  methods: {
