<template>
  <div id="hot" class="animate__animated animate__slideInLeft">
    <div class="header">
      <div class="titleImg">
        <img src="https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=3371462867,2791560787&fm=26&gp=0.jpg" alt="" style="width:25px;height:25px" />
      </div>
    ...
    <div class="main">
      <h3 v-show="!TXZNewsList.site.subs[0].items.length" class="loading">努力加载中...</h3>
      <ul class="hotWeiboList">
        <li
          v-for="(item, index) in TXZNewsList.site.subs[0].items.slice(
            currentIndex,
        ...
        >
          ...
          <a :href="`https://www.so.com/s?ie=utf-8&src=zhishu&q=${item.title}`" target="_blank">{{ item.title }}</a>
...
export default {
  ...
  computed: {
    ...mapState({
      TXZNewsList: state => state.anyknow.TXZNewsList
    })
  },
  methods: {
