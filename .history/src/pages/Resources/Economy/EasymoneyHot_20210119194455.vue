<template>
  <div id="hot" class="animate__animated animate__bounceIn">
    <div class="header">
      <div class="titleImg">
        <img :src="EasymoneyNewsList.site.attrs.logo" alt="" style="width:50px;height:25px" />
      </div>
    ...
    <div class="main">
      <h3 v-show="!EasymoneyNewsList.site.subs[0].items.length" class="loading">努力加载中...</h3>
      <ul class="hotList">
        <li
          v-for="(item, index) in EasymoneyNewsList.site.subs[0].items.slice(
            currentIndex,
        ...
        >
          ...
          <a :href="`https://search.caixin.com/newsearch/search?keyword=${item.title}`" target="_blank" class="mainText">{{
            item.title
          }}</a>
...
export default {
  ...
  computed: {
    ...mapState({
      EasymoneyNewsList: state => state.anyknow.EasymoneyNewsList
    })
  },
  methods: {
