<template>
  <div id="hot" class="animate__animated animate__fadeIn">
    <div class="header">
      <div class="titleImg">
        <img :src="GameSkyGameList.site.attrs.logo" alt="" style="width:50px;height:25px" />
      </div>
    ...
    <div class="main">
      <h3 v-show="!GameSkyGameList.site.subs[0].items.length" class="loading">努力加载中...</h3>
      <ul class="hotList">
        <li
          v-for="(item, index) in GameSkyGameList.site.subs[0].items.slice(
            currentIndex,
        ...
        >
          ...
          <a
            :href="`http://so.gamersky.com/?s=${item.title}`"
            target="_blank"
            class="mainText"
            >{{ item.title }}</a
          >
...
export default {
  ...
  computed: {
    ...mapState({
      GameSkyGameList: state => state.anyknow.GameSkyGameList
    })
  },
  methods: {
